...
def _react_wrapped_function(value_node, source_bytes: bytes | None = None) -> any:
    """Return the inner function expression wrapped by ``memo(...)`` or ``forwardRef(...)``.

    React components are often exported as ``const X = memo((props) => ...)``.
    If the variable value is a call to ``memo``/``forwardRef`` and the first
    ...
    if callee is None:
        return None
    name = _decode(source_bytes, callee) if source_bytes is not None else ""
...
def _outer_declaration_node(node, language: str, source_bytes: bytes | None = None):
    ...
    if node.type == "variable_declarator" and node.parent is not None:
        ...
        func_value = (
            ...
            else _react_wrapped_function(value, source_bytes)
        )
