...
class HumanReviewTracker:
    ...
    def generate_review_summary(self, lang: str = "en") -> str:
        from agent.report_formatter import format_human_review_queue

        return format_human_review_queue(self.queue(), lang=lang)

    def export_review_markdown(self, lang: str = "en") -> str:
        summary = self.generate_review_summary(lang=lang)
        return "\n".join(
            [
                "<!-- mumei-human-review -->",
                summary,
                "",
                "> Generated by mumei-agent HumanReviewQueue for GitHub PR review.",
            ]
        )
    ...
    def reject_review(self, atom_name: str, reviewer: str, notes: str) -> JsonDict:
        entry = self._find_atom(atom_name)
        current = entry.get("status")
        if current == ReviewStatus.ESCALATED_TO_LEAN.value:
            raise ValueError(
                f"cannot reject atom {atom_name!r}: current status is {current}"
            )
...
HumanReviewQueue = HumanReviewTracker
