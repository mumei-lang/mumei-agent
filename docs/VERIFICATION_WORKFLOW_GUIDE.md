...
| `trusted_atoms` | Advisory warnings listing `trusted atom` declarations found in `.mm` sources under the audited path — they bypass Z3 verification, so audits surface them instead of passing silently. |
...
`next_steps` is the only handoff into human review. Do not add aliases for `spec_health_issues`, `verification_violations`, `verification_status`, `cross_validation_gaps`, `next_steps`, `migration_hints`, `healed_files`, `heal_errors`, or `trusted_atoms`; downstream docs, MCP responses, and demo JSON should consume those names exactly.
...
この 1 コマンド flow は `audit -> migrate-suggest -> heal` の順に従います。`audit` は `spec_health_issues` / `verification_violations` / `verification_status` / `cross_validation_gaps` / `next_steps` を返し、`migrate-suggest` / `--auto-migrate` は `migration_hints`、`heal` / `--auto-heal` は `healed_files` / `heal_errors` だけを返し、`.mm` ソース内の `trusted atom` 宣言は `trusted_atoms` アドバイザリとして拾います。MCP `scan_and_fix` は同じ契約を使います。
...
- `trusted_atoms`: 監査対象ディレクトリ配下の `.mm` に含まれる `trusted atom` 宣言（Z3 検証をスキップするため警告として個別レビュー対象）。
...
- `audit.trusted_atoms[]`: 監査パス配下の `.mm` ファイル内 `trusted atom` 宣言の警告リスト。
