...
- `defer mu.Unlock()` / `defer func(){ mu.Unlock() }()` / `go func(){ close(ch) }()` forms are checked at registration: a deferred/goroutine `Unlock` on an unlocked mutex or `close` on a nil/closed channel still panics when the literal runs. `*sync.Mutex` / `*sync.RWMutex` nil values use the dedicated `uninit_mutex` kind — every mutex method dereferences its receiver so nil-receiver `mu.Unlock()` panics (and a nil mutex is never recorded as locked).
- Regression gate: `tests/test_dataflow_facts.py` (+33 cases), `tests/test_foreign_code.py` / `tests/test_cross_validation.py` unchanged pass; full suite green.
