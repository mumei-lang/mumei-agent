...
`scan_and_fix` is the MCP spelling of `audit --code-file ... --auto-migrate --auto-heal`; README flowchart, guide steps, CLI help, MCP docstrings, and this roadmap must describe the same audit → migrate-suggest → heal contract and the six fixed keys above without aliases.
