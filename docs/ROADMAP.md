...
| `spec_health_issues` / `verification_violations` / `cross_validation_gaps` | Canonical audit issue partitions: spec-only issues, existing-code bugs, and spec/code gaps. | `audit`, `scan_and_fix` |
| `next_steps` / `healed_files` / `heal_errors` | Canonical audit action and self-healing evidence partitions. | `audit --auto-migrate --auto-heal`, `scan_and_fix` |
| `migration_hints` | `.mm` skeleton suggestions generated by auto-migration. | `audit --auto-migrate`, `scan_and_fix` |
| `cross_validation_gaps` | Spec/code or cross-spec mismatches that still need review/fix. | `audit`, `validate-spec-to-code`, `check_cross_spec_consistency` |

`scan_and_fix` is the MCP spelling of `audit --code-file ... --auto-migrate --auto-heal`; README, CLI help, MCP docstrings, and this roadmap should describe the same audit → migrate-suggest → heal contract.
