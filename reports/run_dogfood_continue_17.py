#!/usr/bin/env python3
"""Run deterministic (no-LLM) mumei-agent audit on the seventeenth batch of OSS files."""
...
OUT_DIR = Path(__file__).parent / "dogfood_continue_17"
SAMPLE_FILE = Path(__file__).parent / "dogfood_continue_17_sample.json"
TIMEOUT = 240


def load_sample() -> list[tuple[str, str, Path]]:
    ...
    for entry in data:
        ...
        items.append((repo, entry["lang"], path))
    return items
    ...
    for repo, language, path in load_sample():
        ...
        try:
            result = subprocess.run(
                [
                    "uv", "run", "python", "-m", "agent", "audit",
                    "--code-file", str(path),
                    "--language", language,
                    "--format", "json",
                ],
                cwd="/home/ubuntu/repos/mumei-agent",
